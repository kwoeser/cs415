...
// Functions having issues withotu declartion
...
// Incorrect printing, Incorrect order
void print_proc_data(){
    for(int i = 0; i < line_count; i++) { 
        ...
        fscanf(fp, "%*d %*s %*c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %ld %ld %*d %*d %d %*d %*u %*u %*d %*u %lu %*u %*u %*u %*u %*u %*u %*u %*u %*u %*u %d ", 
            &utime, &stime, &nice, &vsize, &threads);
...
int main(int argc, char *argv[]) {
    ...
    if((argc == 3) && (strcmp(argv[1], "-f")) != 0) {
        printf("Error wrong input: ./part4 -f <filename>\n");
        exit(EXIT_FAILURE);
    }
